...
task_customer_landing_to_processing = BashOperator(
 ...
 bash_command='aws s3 mv s3://snowflake/firehose/customers/landing/ s3://snowflake/firehose/customers/processing/{0}/ --recursive'.format(batch_id),
 dag=dag
)
 
task_customers_processing_to_processed = BashOperator(
 task_id="customer_processing_to_processed",
 bash_command='aws s3 mv s3://snowflake/firehose/customers/processing/{0}/ s3://snowflake/firehose/customers/processed/{0}/ --recursive'.format(batch_id),
 dag=dag
)
 
task_orders_landing_to_processing = BashOperator(
 task_id="orders_landing_to_processing",
 bash_command='aws s3 mv s3://snowflake/firehose/orders/landing/ s3://snowflake/firehose/orders/processing/{0}/ --recursive'.format(batch_id),
 dag=dag
)
 
task_orders_processing_to_processed = BashOperator(
 task_id="orders_processing_to_processed",
 bash_command='aws s3 mv s3://snowflake/firehose/orders/processing/{0}/ s3://snowflake/firehose/orders/processed/{0}/ --recursive'.format(batch_id),
 dag=dag
)
 
snowflake_query_orders = [
    """copy into DB1.SCHEMA1.ORDERS
...
( select t.$1,t.$2,t.$3,t.$4,t.$5,t.$6,t.$7,t.$8,t.$9,'{0}' from @ORDERS_STAGE t);""".format(batch_id),
]

snowflake_query_customers = [
    """copy into DB1.SCHEMA1.CUSTOMERS
(C_CUSTKEY, C_NAME, C_ADDRESS, C_NATIONKEY, C_PHONE, C_ACCTBAL, C_MKTSEGMENT, C_COMMENT, BATCH_ID) from
( select t.$1,t.$2,t.$3,t.$4,t.$5,t.$6,t.$7,t.$8,'{0}' from @CUSTOMER_STAGE t);""".format(batch_id),
]
